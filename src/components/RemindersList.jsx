...
export default function RemindersList({ announcements, completedActions }) {
  ...
  return (
    <div id="reminders-list" className="bg-slate-900 rounded-3xl p-6 shadow-xl text-white relative overflow-hidden">
      ...
      {activeReminders.length === 0 ? (
        <div className="text-center py-6 bg-slate-800/40 rounded-2xl border border-dashed border-slate-800">
          ...
          <p className="text-[11px] text-slate-400 mt-1">Excellent! No high priority deadlines left within the next 7 days.</p>
        </div>
      ) : (
          ...
          {activeReminders.map(({ ann, daysRemaining }, idx) => {
            ...
            return (
              <motion.div
                ...
                className="bg-slate-850 p-4 rounded-2xl border border-slate-800 flex flex-col justify-between"
              >
